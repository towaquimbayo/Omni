...
WEBHOOK_URL = "http://64.180.57.134:8123/api/webhook/-FT1pChLFScJr6Xhw8dPOgd0s"
INTERFERENCE_THRESHOLD = 35  # Define an appropriate threshold for interference
LAST_POST_TIME = 0
...
def send_interference_alert(interference_level):
    global LAST_POST_TIME
    current_time = time.time()
    if current_time - LAST_POST_TIME >= 5:  # Ensure at least 0.3 minute between POST requests
        try:
            response = requests.post(WEBHOOK_URL, json={"interference_level": float(interference_level)})
            response.raise_for_status()
            print(f"POST request sent to webhook: {response.status_code}")
            LAST_POST_TIME = current_time
        except requests.RequestException as e:
            print(f"Failed to send POST request: {e}")

def check_interference(csi_amplitude_array):
    # Example logic to calculate interference (adjust as needed for your use case)
    interference_level = np.mean(csi_amplitude_array[-1])  # Calculate mean of the latest row
    if interference_level > INTERFERENCE_THRESHOLD:
        send_interference_alert(interference_level)
...
class csi_data_graphical_window(QWidget):
    ...
    def update_data(self):
        ...
        check_interference(self.csi_amplitude_array)
